def define_3ph_pvsystem_with_transformer(dss, bus, kv, kva, pmpp):
    ...
    dss.text("New transformer.PV_{} phases=3 windings=2 buses=(PV_sec_{}, PV_ter_{}) conns=(wye, wye) kVs=({},0.48) "
             "xhl=5.67 %R=0.4726 kVAs=({}, {})".format(bus, bus, bus, kv * 1.73, kva, kva))
    ...
    dss.text("New PVSystem.PV_{} phases=3 conn=wye  bus1=PV_ter_{} kV=0.48 kVA={} Pmpp={} pf=1 %cutin=0.00005 %cutout=0.00005 VarFollowInverter=No"
             " effcurve=Myeff  P-TCurve=MyPvsT Daily=MyIrrad  TDaily=MyTemp vmaxpu=1.3".format(bus, bus, kva, pmpp))
...
def define_3ph_generator_with_transformer(dss, bus, kv, kva, kw):
    ...
    dss.text("New generator.PV_{} phases=3 conn=wye  bus1=PV_ter_{} kV=0.48 kw={} pf=1 model=1 vmaxpu=1.3".format(bus, bus, kw))